...
interface FollowerUser extends DbProfile {}
    ...
    supabase
      ...
      .then(({ data }) => {
        if (data) {
          ...
          setFollowers((data as any[]).map((row) => row.follower).filter(Boolean) as FollowerUser[]);
        }
        setLoading(false);
      });
  }, [id]);
      ...
      <main className="max-w-2xl mx-auto px-4 py-8">
        ...
        <div className="bg-white rounded-2xl border border-[#E4E4E7]" style={{ boxShadow: "0 1px 3px 0 rgb(0 0 0 / 0.07)" }}>
          <div className="px-6 py-4 border-b border-[#F5F4F1] flex items-center gap-2">
            <Users size={16} style={{ color: "#0BBFB5" }} />
          ...
          {loading ? (
            ...
            <div className="py-16 text-center text-[#A1A1AA]">
              ...
              <div className="font-medium text-[#1C1C1E]">Нет подписчиков</div>
              <div className="text-sm mt-1">
                {isOwnProfile ? "На тебя пока никто не подписан" : "На этого пользователя пока никто не подписан"}
              </div>
            </div>
          ) : (
            <div className="divide-y divide-[#F5F4F1]">
                  ...
                  <div key={f.id} className="px-6 py-4 flex items-center gap-3">
                    ...
                    {!isMe && user && (
                      <button
                        ...
                        {following ? <><UserCheck size={13} /> Подписан</> : <><UserPlus size={13} /> Подписаться</>}
                      </button>
                    )}
                  </div>
                );
